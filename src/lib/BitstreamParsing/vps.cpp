...
vps::vps(const uvgvpcc::Parameters& paramUVG, const std::shared_ptr<uvgvpcc::GOF> gofUVG)
{
    if (paramUVG.useEncoderKvazaar || paramUVG.useEncoderCommand) {
        codec_group_ = 1;  // TMC2 : CODEC_GROUP_HEVC_MAIN10
    } else if (paramUVG.useEncoderUvg266) {
    ...
    for (uint8_t k = 0; k < (vps_atlas_count_minus1_ + 1); k++) {
        ...
        vps_map_count_minus1_.push_back(vps_map_count_minus1);
        ...
        vps_map_predictor_index_diff_.push_back({false});
        ...
        for(uint8_t i = 1; i <= vps_map_count_minus1_.at(k); ++i) {
            ...
            if(vps_map_absolute_coding_enabled_flag_.at(k).at(i) == 0) {
                vps_map_predictor_index_diff_.at(k).push_back(false);
        ...
        // TODO: codec_id same as codec_group_idc? 
        ...
        gi.gi_geometry_2d_bit_depth_minus1 = uint8_t(paramUVG.geometryNominal2dBitdepth - 1);
        gi.gi_geometry_MSB_align_flag = false;
        gi.gi_geometry_3d_coordinates_bit_depth_minus1 = uint8_t(paramUVG.geoBitDepthInput); // no -1 because it is already -1 from what is should be
        ...
        for (uint8_t i = 0; i < ai.ai_attribute_count; i++) {
            ...
            ai.ai_attribute_dimension_minus1.push_back(2); // TODO: 2 comes from bitstreamGenInterface, why?
            uint8_t d = ai.ai_attribute_dimension_minus1.at(i);
            ...
            ai.ai_attribute_dimension_partitions_minus1.push_back(0); // TODO; default value, is it ok?
            ...
            ai.ai_attribute_2d_bit_depth_minus1.push_back(7); // TODO: get this from paramUVG?
    ...
    vps_length_bytes_ = std::ceil(float(vps_length_bits) / 8);
...
bool vps::write_vps(bitstream_t* stream)
{
    ...
    // TODO: Do we need profile_toolset_constraints_information? Not present from TMC2 interface
    ...
    for (uint8_t j = 0; j < (vps_atlas_count_minus1_ + 1); j++) {
        ...
        if (vps_geometry_video_present_flag_.at(j)) {
            ...
            WRITE_U(stream, geometry_info_.at(j).gi_geometry_3d_coordinates_bit_depth_minus1, 5, "gi_geometry_3d_coordinates_bit_depth_minus1");
        ...
        if (vps_attribute_video_present_flag_.at(j)) {
            ...
            for (uint8_t i = 0; i < attribute_info_.at(j).ai_attribute_count; ++i) {
                ...
                if(vps_map_count_minus1_.at(j) > 0) {
                    WRITE_U(stream, int(attribute_info_.at(j).ai_attribute_map_absolute_coding_persistence_flag.at(i)), 1, "ai_attribute_map_absolute_coding_persistence_flag");
                }
                ...
                if(d == 0) { //true
                    ...
                    WRITE_U(stream, attribute_info_.at(j).ai_attribute_dimension_partitions_minus1.at(i), 6, "ai_attribute_dimension_partitions_minus1");
                }
                ...
                for (uint8_t k = 0; k < m; k++) {
                    if(k + d == m) {
                        ...
                        WRITE_UE(stream, attribute_info_.at(j).ai_attribute_partition_channels_minus1.at(i).at(k), "ai_attribute_partition_channels_minus1");
                    }
                    d -= n + 1;
                }
...
profile_tier_level vps::fill_ptl(size_t &len)
{
    profile_tier_level ptl;
    ptl.ptl_tier_flag = 0;                  // TODO: Check if 1 should be used instead
    ptl.ptl_profile_codec_group_idc = codec_group_;
    ptl.ptl_profile_toolset_idc = 1;        // V-PCC Extended, TODO: Add V-PCC Basic
    ptl.ptl_profile_reconstruction_idc = 1; // Rec0 reconstruction profile, TODO: Should this be 255?
    ptl.ptl_max_decodes_idc = 15;           // TODO: 15 comes from tmc2, why? is it correct?
    ...
    size_t ptc_len = 40 + (ptl.ptc.ptc_num_reserved_constraint_bytes * 8);
