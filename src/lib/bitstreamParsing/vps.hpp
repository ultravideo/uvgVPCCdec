...
class vps {
   public:
    ...
    //vps(const uvgvpcc_dec::Parameters& paramUVG, const std::shared_ptr<uvgvpcc_dec::GOF>& gofUVG);

    /* Write the VPS into a bitstream form */
    bool read_vps(bitstream_t* stream);

    size_t get_vps_byte_len() { return vps_length_bytes_; };
    void set_vps_byte_len(size_t vps_length_bytes) { vps_length_bytes_ = vps_length_bytes; };
    uint8_t get_map_count(size_t index) { return vps_map_count_minus1_.at(index) + 1; };
